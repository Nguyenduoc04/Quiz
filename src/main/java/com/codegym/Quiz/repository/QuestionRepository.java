...
public interface QuestionRepository extends JpaRepository<Question, Long> {
    ...
    Page<Question> findByCreatedByOrderByCreatedAtDesc(User createdBy, Pageable pageable);

    /** Phân trang câu hỏi theo danh mục */
    Page<Question> findByCategoryIdOrderByCreatedAtDesc(Long categoryId, Pageable pageable);

    /** Phân trang câu hỏi theo danh mục và giáo viên */
    Page<Question> findByCategoryIdAndCreatedByOrderByCreatedAtDesc(Long categoryId, User createdBy, Pageable pageable);

    /** Tìm kiếm câu hỏi toàn hệ thống theo từ khóa (nội dung câu hỏi) */
    @Query("""
            SELECT q FROM Question q
            WHERE LOWER(q.content) LIKE LOWER(CONCAT('%', :keyword, '%'))
            ORDER BY q.createdAt DESC
            """)
    Page<Question> searchAllByKeyword(@Param("keyword") String keyword, Pageable pageable);

    /** Tìm kiếm câu hỏi theo nâng cao: keyword, categoryId, difficulty */
    @Query("""
            SELECT q FROM Question q
